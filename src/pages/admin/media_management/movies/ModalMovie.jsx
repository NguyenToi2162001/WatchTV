...
const ModalMovie = ({ open, handleClose, handleChoose, movie, handleSelect, setMovie, handleSubmit, errors }) => {
    ...
    return (
        <div>
            <Dialog open={open} onClose={handleClose} sx={{ '& .MuiDialog-paper': { width: '80%', maxWidth: 'none' } }}>
                <DialogTitle>{movie.id ? "Edit Author" : "Add Author"}</DialogTitle>
                <Grid container spacing={2} className='px-3' alignItems="stretch">
                    ...
                    <Grid item xs={12} md={6} style={{ display: 'flex' }}>
                        <Paper style={{ padding: 16 }}>
                            ...
                            {getObjectById(movie?.planID, plans)?.lever > 2 && <TextField
                                margin="dense"
                                label="RentalPrice"
                                type="text"
                                fullWidth
                        ...
                        <Paper style={{ padding: 16 }} className='flex-1'>
                            <div >
                                ...
                                {movie.listCate.map((row) => (
                                    <button onClick={() => handleDelete(row, "categories")} className="p-2 ms-5 mt-2 bg-cyan-400" style={{ position: 'relative' }}>{getObjectById(row, categories)?.nameCategory}
