...
function ModalChoose({ openChoose, dataChoose, setOpenChoose, chooseType, onSelect, selectedItems }) {
    ...
    const handleSelect = (item) => {
        onSelect(item, chooseType); // Pass the selected item back to the parent component
    };
    const isSelected = (item) => selectedItems.includes(item);
    const [searchTerm, setSearchTerm] = useState('');
    const filtered = dataChoose?.filter((item) => item.nameCategory?.toLowerCase().includes(searchTerm.toLowerCase())
